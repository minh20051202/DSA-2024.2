...
class GraphEdge(Generic[VT, ET]):
    ...
    def __str__(self):
        """
        Trả về biểu diễn chuỗi của cạnh.
        
        Trả về:
            str: Chuỗi mô tả cạnh với các thuộc tính
        """
...
class GraphVertex(Generic[VT, ET]):
    ...
    def __str__(self):
        """
        Trả về biểu diễn chuỗi của đỉnh.
        
        Trả về:
            str: Chuỗi mô tả đỉnh
        """
    ...
    def __find_cycles_dfs_util_with_edges__(
        ...
        if not u_vertex:
            if recursion_stack_nodes.contains_key(u_data): 
                recursion_stack_nodes.remove(u_data) # Đảm bảo stack được dọn dẹp
            return
        ...
        for edge_to_v in u_vertex.edges: 
            ...
            if not globally_visited.contains_key(v_data):
                # Đỉnh đích chưa được thăm, tiếp tục DFS
                self.__find_cycles_dfs_util_with_edges__(
                ...
                # Tìm vị trí bắt đầu chu trình trong path_edges
                ...
                # Trích xuất chu trình từ điểm bắt đầu đến cuối path
                if start_cycle_edge_index != -1:
                    ...
                    if path_node_at_cycle_start and path_node_at_cycle_start.data.source == v_data:
                        # Tạo danh sách tạm để lưu các cạnh của chu trình
                        temp_cycle_edges_list = Array[GraphEdge[VT, ET]]()
                        current_edge_node_in_path = path_node_at_cycle_start
                        while current_edge_node_in_path:
                            temp_cycle_edges_list.append(current_edge_node_in_path.data)
                            current_edge_node_in_path = current_edge_node_in_path.next
                        
                        # Chuyển đổi Array thành LinkedList
                        cycle_edges = LinkedList[GraphEdge[VT, ET]]()
                        for i in range(temp_cycle_edges_list.size):
                            cycle_edges.append(temp_cycle_edges_list.get(i))
                        
                        # Thêm chu trình vào danh sách kết quả
                        all_cycles_edges.append(cycle_edges)
                        break
            
            # Dọn dẹp: loại bỏ cạnh vừa thêm khỏi path nếu nó vẫn là cạnh cuối cùng
            if not path_edges.is_empty() and path_edges.get_last() == edge_to_v:
                path_edges.remove_last() 

        # Loại bỏ đỉnh hiện tại khỏi recursion stack khi hoàn thành xử lý
        if recursion_stack_nodes.contains_key(u_data):
