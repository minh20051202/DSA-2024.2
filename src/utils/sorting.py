...
def _split_linked_list(source_list: 'LinkedList[T]') -> tuple['LinkedList[T]', 'LinkedList[T]']:
    """Chia một LinkedList thành hai nửa. Trả về tuple chứa hai LinkedList mới."""
    if source_list.is_empty() or source_list.head is None or source_list.head.next is None:
        # Trả về danh sách gốc là nửa đầu, và một danh sách rỗng là nửa sau nếu không thể chia
        copy_source = LinkedList[T]()
        for item in source_list: # Tạo bản sao để tránh thay đổi list gốc
            copy_source.append(item)
        return copy_source, LinkedList[T]()

    slow = source_list.head
    fast = source_list.head.next

    # Di chuyển fast hai bước và slow một bước
    while fast is not None and fast.next is not None:
        slow = slow.next
        fast = fast.next.next
    
    # slow giờ đang ở cuối nửa đầu (hoặc gần cuối)
    left_half = LinkedList[T]()
    right_half = LinkedList[T]()
    current = source_list.head
    
    # Sao chép nửa đầu
    while current is not None and current != slow.next:
...
def merge_sort_linked_list(linked_list_to_sort: 'LinkedList[T]', comparator: Callable[[T, T], bool] | None = None) -> 'LinkedList[T]':
    ...
    """
    if linked_list_to_sort.is_empty() or linked_list_to_sort.get_length() <= 1:
