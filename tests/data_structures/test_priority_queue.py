...
class TestPriorityQueue(unittest.TestCase):
    ...
    def test_string_representation(self):
        pq_str = PriorityQueue[str]()
        pq_str.enqueue("one", 1)
        pq_str.enqueue("two", 2)
        # Min-heap: one, sau đó two
        # Expected: PriorityQueue([one, two]) hoặc PriorityQueue([two, one]) tùy thuộc vào thứ tự bên trong của Array
        # Với Array.__iter__ duyệt từ đầu, thứ tự phụ thuộc vào trạng thái của heap array.
        # Sau khi enqueue 1 rồi 2, heap là [ (one,1), (two,2) ].
        # str(pq_str) sẽ là PriorityQueue([one, two])
        self.assertEqual(str(pq_str), "PriorityQueue([one, two])") 
        self.assertEqual(repr(pq_str), "PriorityQueue(kich_thuoc=2)")

        pq_int = PriorityQueue[int]()
        pq_int.enqueue(100,100)
        pq_int.enqueue(5,5)
        pq_int.enqueue(20,20)
        # Heap: [(5,5), (100,100), (20,20)] -> heapify_up(20) -> [(5,5), (100,100), (20,20)] (no swap)
        # Sau đó heapify_up(100) -> [(5,5), (100,100), (20,20)] (no swap)
        # Array iter order is 0, 1, 2. Items are 5, 100, 20
        # The __str__ iterates the underlying array in order. 
        # For a min-heap [ (5,P5), (100,P100), (20,P20) ], this is the order
        self.assertEqual(str(pq_int), "PriorityQueue([5, 100, 20])") 
