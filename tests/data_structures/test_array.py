import unittest
from src.data_structures.array import Array # Import lớp Array đã tạo
...
class TestArray(unittest.TestCase):
    ...
    def test_str_and_repr(self):
        """Kiểm tra biểu diễn chuỗi __str__ và __repr__."""
        arr = Array[int](3)
        self.assertEqual(str(arr), "[]")
        self.assertEqual(repr(arr), "Array([], size=0, capacity=3)")

        arr.append(1)
        arr.append(2)
        self.assertEqual(str(arr), "[1, 2]")
        self.assertEqual(repr(arr), "Array([1, 2], size=2, capacity=3)")

        arr_str = Array[str](2)
        arr_str.append("hello")
        self.assertEqual(str(arr_str), "[hello]")
        self.assertEqual(repr(arr_str), "Array(['hello'], size=1, capacity=2)")
    ...
    def test_insert(self):
        ...
        self.assertEqual(str(arr), "[5, 10, 20, 30]")
        self.assertTrue(arr.capacity >= 4)

        arr.insert(4, 40) # [5, 10, 20, 30, 40]
        self.assertEqual(len(arr), 5)
        self.assertEqual(str(arr), "[5, 10, 20, 30, 40]")
